...
PSF_cell_folder = [local_path,'\PSF_cell\'];
...
pixel_length = 1.60; %um
% sample interval of field in millimeters
fld_sample_interval = 0.02;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% sensor response integral path
wave_dist_path = [local_path,'\wav_response\wav_dist_cell_avr_itvl_380_10nm_780.mat'];
...
% illumination_path = [local_path,'\illumination_info\illumination_info.xlsx'];
...
for h_index = tile_length/2 : tile_length : img_h/2 
    for w_index = tile_length/2 : tile_length : img_w/2 %1/4个区域
        ...
        fld_sample_dist = sqrt((fld_sample_delta_h)^2 + ...
                               ...
                               pixel_length * 0.001; %分区中心距离图像中心的距离，单位mm
        ...
        [main_center_h, PSF_fld_tmp, PSF_wav_tmp] = judge_main_wav(fld_sample_prepath, fld_index_int);
        %视场，~，主光线波长
        ...
        for wave_index = 1:wave_num
            ...
            PSF_space = wave_txt{6};
            PSF_space_tmp = split(PSF_space, ' ');
            PSF_center = wave_txt{12};
            ...
            data_cell_length = round(pixel_length / PSF_data_space); %每个像素覆盖几个采样点
            % the distance of PSF to the center of image
            PSF_center_h = str2double(PSF_center_tmp{10});%origin 8
            ...
            for rotat_index = 1:4
                ...
                pad_pixel_num = ceil((pixel_num * data_cell_length - PSF_H)/2);
                % first pad the array
